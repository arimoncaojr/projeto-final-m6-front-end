...
export const Card = ({post , type}:IPostCardProps) => {
     ...
     const description = post.description[0].toUpperCase() + post.description.slice(1).toLowerCase()
     const isActive = post.isActive
     const isGoodPurchase = post.isGoodPurchase
     const km = post.kilometers
     const year = post.year
     const price = Number(post.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
     const user = post.user.name.replace(/\b\w{1}/g, (match) => match.toUpperCase());
     ...
     return (
          <CardStyle isActive={isActive}>
               <figure>
                    ...
                    {type !== "home" ? isActive? <span>Ativo</span> : <span>Inativo</span> : false}
               </figure>
               <h2>{nameCar}</h2>
               <p>{description}</p>
